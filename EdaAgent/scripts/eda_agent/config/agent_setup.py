...
from ..tools.full_correlation_tool import full_correlation_tool
from ..tools.regression_tool import regression_tool
from ..tools.stations_tool import stations_tool
...
def build_agent(modelo: str = "razonamiento", temperature: float = 0):

    if not isinstance(modelo, str):
        raise ValueError("modelo debe ser string")

    llm = build_llm(modelo=modelo, temperature=temperature)

    checkpointer = InMemorySaver()

    try:
        log_agent_start(modelo=modelo, n_tools=len(TOOLS))
    except Exception:
        pass 
